...
**v0.1.3 shipped (2026-05-15). Release workflow fix: manual .sig upload + latest.json generation to work around tauri-action@v0 updater bug. v0.1.2 (2026-05-15): full 6-domain codebase audit — CSP enabled, withGlobalTauri disabled, async-safe git polling, MCP spec fixes, bus_tail leak fix, FitAddon disposal, CSS variable consistency, Linux build fix.** Phase 8 (Index integration tab + graph view) CLOSED — all subphases 8.0-8.7 shipped; D-019 remade IndexGraph as vault browser list (D3 deps removed). Visual maturity pass (2026-05-19): 5-layer UI polish — card-shaped tab containment, spacing/type design tokens (`--section-header-*`, `--radius-md`), panel containment across all notification tabs, IndexGraph recents-first section + kind chip filters, unified `.rift-btn` button system. Feature-agent batch (2026-05-20): bus event sparklines (SparklineChart + SparklineBuffer, 60s circular buffer in BusTail status header), keyboard shortcut overlay (ShortcutOverlay + keybindings.ts, Ctrl+?), tab workspace persistence (promoted tab state saved/restored via localStorage), session replay viewer, command palette (Ctrl+K), terminal search (Ctrl+Shift+F). Post-v1 work active: D-014 MCP tool surface (20 tools, Phases A-D complete), D-018 live lane classification (closed C-023), D-020 temporal activity heatmap (open — see DEFERRED.md). Remaining deferrals: D-010 (Sentinel, post-v1 by design), D-012 (StatusLine — SESSION elapsed timer live, SKILL/EFFORT wired to CC StatusJSON polling with aegis bus override; CTX/USE/WEEK were already live), D-020 (heatmap — Medium-tier candidate, post-v1). 14 MEDIUM + 12 LOW + 4 doc-drift audit findings remain per AUDIT_2026-04-27.md (top: silent error handling, missing loading states, accessibility; CRITICAL + HIGH all fixed in v0.1.2). Vision and architecture locked at v0.6 (2026-04-27); §10.15 real-time-update mechanism LOCKED via `decisions/§10.15_real-time_update_mechanism.md`; §10.17 agent tab grouping resolved via `decisions/§10.17_agent_tab_grouping_filtering.md`. Workspace has four Rust crates (`crates/rift-bus` — protocol/transport/translators, `crates/rift-cli` — `rift hook`/`rift status` CLI, `crates/rift-core` — PTY abstraction, `crates/rift-aegis` — private optional feature-gated path dep) + `crates/rift-mcp` (MCP translator), the `src-tauri/` Tauri shell, and the Svelte 5 frontend under `src/`. §9 Integration Decoupling is CI-enforced. All 3 mockups (terminal alone / GUI alone detached / integrated cockpit) live; see `DEFERRED.md` closed-deferrals section for ship history.
