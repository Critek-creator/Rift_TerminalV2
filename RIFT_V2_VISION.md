...
### 10.15 Locked — Real-time update mechanism

Resolved 2026-04-26. Two-tier architecture, see `decisions/§10.15_real-time_update_mechanism.md` for the full spec.

- **Tier 1 (in-process)** — `tauri::ipc::Channel<T>` for high-volume per-stream traffic (PTY bytes), Tauri events for fan-out/window-targeted signals.
- **Tier 2 (cross-process)** — `tokio::sync::broadcast` ring buffer + UDS / named-pipe IPC server (`crates/rift-bus/src/ipc.rs`) for translators living outside the Tauri process. 4-byte LE length-prefixed frames, 16 MiB cap, replay-on-reconnect.

Implemented across Phase 4.1-4.3 (envelope schema, RiftBus, IpcServer/IpcClient). Closed deferrals: C-001, C-006, C-007.
...
- **v0.6 (2026-04-27)** — §10.15 real-time update mechanism flipped from Open → Locked, referencing the two-tier resolution doc at `decisions/§10.15_real-time_update_mechanism.md`. No design changes; doc-side patch reflecting Phase 4.x ship state. §10.17 (agent tab grouping/filtering) resolved doc-only via `decisions/§10.17_agent_tab_grouping_filtering.md` per Phase 7.6 brainstorm; staged plan, no code in v1. §10.18 (graph library + rendering) actively resolving across Phase 8 — D3 force layout shipped at 8.3, library decision deferred-as-shipped under SVG+d3 stack. All v1 active deferrals closed except D-010 (Sentinel — explicitly post-v1 by design).
