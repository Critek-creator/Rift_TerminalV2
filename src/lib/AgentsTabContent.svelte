<script lang="ts">
  ...
  function handleEnvelope(env: Envelope) {
    ...
    switch (env.kind) {
      ...
      // Other agent kinds pass through — they bump totalEvents (which is
      // enough to flip the §10.7 detected gate at the App-level master sub),
      // but the tab itself only renders start/activity/end.
      default:
        break;
    }
  }
...
<style>
  ...
  .strip {
    ...
    background: linear-gradient(to bottom, rgba(176, 120, 232, 0.04), transparent);
    color: var(--amber-dim);
    font-size: 10px;
    letter-spacing: 0.1em;
    overflow: hidden;
  ...
  .capability-card {
    ...
    padding: 12px 14px;
    background: rgba(176, 120, 232, 0.03);
    color: var(--amber-warm);
    font-size: 11px;
    line-height: 1.55;
  }
  ...
  .card {
    border: 1px solid var(--term-purple);
    background: rgba(176, 120, 232, 0.04);
    padding: 8px 10px;
    margin-bottom: 6px;
    display: flex;
    flex-direction: column;
  ...
  .card.inactive {
    border-color: var(--amber-faint);
    background: rgba(168, 120, 48, 0.04);
  }
  .card.cancelling {
    border-color: var(--amber-faint);
    opacity: 0.75;
